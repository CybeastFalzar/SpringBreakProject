...
SET ANSI_NULLS ON
GO
CREATE VIEW [Reference].[uvw_MarketingCategories] AS
WITH C1 AS (SELECT DISTINCT
	MK.MakeID,
	MD.ModelID,
	MK.MakeName,
	MD.ModelName,
...
WHERE Mk.MakeName = 'Citroen' OR MK.MakeName = 'Delahaye' 
),
C2 AS(
SELECT DISTINCT
	MK.MakeID,
...
FROM Data.Model AS MD
	INNER JOIN Data.Make AS MK
	ON MD.MakeID = MK.MakeID
WHERE MK.MakeName = 'Delorean' OR MK.MakeName = 'Jaguar' 
),
C3 AS(
SELECT DISTINCT
	MK.MakeID,
...
FROM Data.Model AS MD
	INNER JOIN Data.Make AS MK
	ON MD.MakeID = MK.MakeID
WHERE MK.MakeName = 'Porsche' OR MK.MakeName = 'Aston Martin' OR MK.MakeName = 'Mercedes' OR MK.MakeName = 'BMW'
),
C5 AS(
SELECT DISTINCT
	MK.MakeID,
...
FROM Data.Model AS MD
	INNER JOIN Data.Make AS MK
	ON MD.MakeID = MK.MakeID
WHERE MK.MakeName = 'Maybach' OR MK.MakeName = 'Rolls Royce' OR MK.MakeName = 'Mercedes' OR MK.MakeName = 'BMW' 
),
C6 AS(
SELECT DISTINCT
	MK.MakeID,
